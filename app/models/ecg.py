...
class ECG(Base):
    ...
    @classmethod
    async def get_by_id(cls, session: AsyncSession, ecg_id: str, auth_user: User) -> ECG | None:
        stmt = select(cls).where(cls.id == ecg_id, cls.owner_id == auth_user.id)
        return await session.scalar(stmt.order_by(cls.id))

    @classmethod
    async def create(
        cls, session: AsyncSession, ecg_id: str, date: datetime.datetime, leads: list[LeadSchema], auth_user: User
    ) -> ECG:
        ecg = ECG(id=ecg_id, date=date, leads=leads, owner_id=auth_user.id)
        session.add(ecg)
        await session.flush()
